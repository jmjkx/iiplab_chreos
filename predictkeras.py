...
if __name__ == '__main__':
    ...
    color_dict = {0: (0, 0, 0),
                  ...
                  8: (255, 255, 255)}
    y_pred_test_img = []
    preprocess = sm.get_preprocessing('mobilenetv2')
#    get_custom_objects().update({'swish_activation': Activation(swish_activation)})
   # model = load_model("/workspace/model_treffunet_0923_best.h5", custom_objects={'swish': tf.nn.swish})
    model = load_model("/workspace/model_treffunet_0924_b0_best.h5", custom_objects={'iou_score': sm.metrics.iou_score})
    for img_name in input_imgs:

        #tif = TIFF.open(img_name)
        image = cv2.imread(img_name)
        i = model.predict(image.reshape(1,512,512,3))
        i = preprocess(i)
