...
if __name__ == '__main__':
    ...
    for img_name in input_imgs:
        ...
        with torch.no_grad():
            ...
            img = img1 + img2.reshape(512,512,9)
            m_pre = rgb(img, color_dict)
            m2_pre = rgb(img2.reshape(512,512,9), color_dict)
            for i in range(512):
                for j in range(512):
                    if (m2_pre[i,j] == np.array([255,0,255])).all():
                        m_pre[i,j] == np.array([255,0,255])
                    if (m2_pre[i, j] == np.array([0, 0, 0])).all():
                        m_pre[i, j] == np.array([0, 0, 0])

            out_img = output_path + '/' + os.path.split(img_name)[1].split('.')[0] + '_gt.png'
            cv2.imwrite(out_img,  m2_pre)
